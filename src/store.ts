...
/** lowercase, hyphen-separated; reserved word "project" never returned as-is (plan decision 7). */
...
export class Store {
  ...
  /** Create a task: writes meta.json only — no record, version 0 (plan decision 1). */
   ...
   * everything else is never a rejection reason (plan decision 9). Project scope
  ...
  async append(taskId: string, input: AppendInput): Promise<AppendResult> {
    ...
    return this.enqueue(async () => {
      ...
      if (!meta) {
        ...
        // Project scope lifecycle: no create, auto-create dir+meta on first append (plan decision 7).
  ...
  /**
   * List tasks by scanning tasks/ directories — no index.json (plan decision 5).
