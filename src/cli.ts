...
import { KNOWN_ROLES, resolveAgent, resolveRole } from "./workspace.js";
import {
  hubCreate,
  hubDecide,
  hubList,
...
export type ParsedArgs =
  ...
  | { command: "start-next"; task_id?: string; url: string; force: boolean }
  ...
  | { command: "new"; requirement: string; pane?: string }
...
function parseStartNext(args: readonly string[]): ParsedArgs {
  const t = tokenize(args, { values: new Set(["url"]), bools: new Set(["force"]) });
  if ("error" in t) return { command: "usage", error: t.error };
  const extra = extraPositionalError(t, 1);
  if (extra !== undefined) return { command: "usage", error: extra };
...
function parseNew(args: readonly string[]): ParsedArgs {
  const t = tokenize(args, { values: new Set(["pane"]) });
  if ("error" in t) return { command: "usage", error: t.error };
  const extra = extraPositionalError(t, 1);
  if (extra !== undefined) return { command: "usage", error: extra };
  const requirement = t.positionals[0];
  if (requirement === undefined || requirement.length === 0) {
    return { command: "usage", error: "new requires a one-sentence requirement" };
  }
  const pane = strFlag(t, "pane");
  return { command: "new", requirement, ...(pane !== undefined ? { pane } : {}) };
}
...
// --- handlers -----------------------------------------------------------------
// All fourteen subcommands are wired (notify; mode/start-next/context;
// ack reuses the context publish path as a fixed human ack note; status is a
// human overview over the context list path).
...
/**
 * skills/ resolved the same module-relative way (../skills — one directory up
 * from src/cli.ts and dist/cli.js alike, always the repo's skills dir).
 * Kickoff prompts embed the ABSOLUTE skill path: the receiving Agent's cwd is
 * the target project, where a cwd-relative "skills/..." does not exist
 * (skills-reachability fix: the path must be absolute, not cwd-relative).
 */
...
async function runStartNext(parsed: Extract<ParsedArgs, { command: "start-next" }>): Promise<number> {
  ...
  const run = await runScript(LAUNCH_SCRIPT, [taskId, role, agent]);
...
/** Kickoff prompt for tut new — same convention as launch.sh's round prompt. */
function newTaskPrompt(requirement: string): string {
  const architectSkill = path.join(SKILLS_DIR, "architect.md");
  return `新任务：${requirement}。请先读 ${architectSkill}，按其中的接手流程完成设计并发布（context.create → context.publish design），完成后告诉我任务 ID。`;
}
 ...
 */
async function runNew(parsed: Extract<ParsedArgs, { command: "new" }>): Promise<number> {
  const paneLabel = parsed.pane ?? (await resolveAgent("architect"));
  const run = await runScript(LAUNCH_SCRIPT, ["--new", paneLabel, newTaskPrompt(parsed.requirement)]);
  if (run.error !== undefined) {
    process.stderr.write(`tut: cannot run launcher ${LAUNCH_SCRIPT}: ${run.error.message}\n`);
    return 1;
  }
  ...
  process.stdout.write(`start-new: delivered to pane '${paneLabel}'\n`);
  return 0;
}
...
async function runDecide(parsed: Extract<ParsedArgs, { command: "decide" }>): Promise<number> {
  try {
    printJson(
      await hubDecide(parsed.url ?? DEFAULT_HUB_URL, {
        task_id: parsed.task_id,
...
async function runAssign(parsed: Extract<ParsedArgs, { command: "assign" }>): Promise<number> {
  // helper overshoots the repo by one level in both layouts (known bug, reported;
  // resolveRole below is only a label fallback, so its misdirected dir merely
  // shifts resolution from routes.json to the identical DEFAULT_ROLES values).
  const file = fileURLToPath(new URL("../scripts/workspace.json", import.meta.url));
  let raw: unknown;
  try {
    raw = JSON.parse(await readFile(file, "utf8"));
  } catch (e) {
    process.stderr.write(`tut: assign: cannot read ${file}: ${(e as Error).message}\n`);
    return 1;
  }
  ...
  // Existing valid label wins; otherwise derive one via the frozen order.
  const currentLabel = (entry as { label?: unknown } | undefined)?.label;
  const label =
    typeof currentLabel === "string" && currentLabel.length > 0
      ? currentLabel
      : (await resolveRole(parsed.role)).label;
  const next =
    entry === undefined || entry === null
      ? { label, agent: parsed.agent }
      : { ...(entry as Record<string, unknown>), label, agent: parsed.agent };
  ...
  process.stdout.write(`assign: ${parsed.role} → ${parsed.agent} (pane label '${label}')\n`);
  return 0;
}
...
export async function main(argv: readonly string[] = process.argv.slice(2)): Promise<number> {
  ...
  switch (parsed.command) {
    ...
    case "new": return HANDLERS.new(parsed);
