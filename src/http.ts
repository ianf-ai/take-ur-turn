 ...
 *   other         → 404 JSON
 *
 * DNS-rebinding guard (decision 3): Host must be 127.0.0.1 / localhost / [::1]
...
/** Only loopback Host values pass (decision 3); absent Host (HTTP/1.0-style clients) is tolerated. */
...
export function createRequestHandler(deps: RequestHandlerDeps): RequestHandler {
  ...
  async function handleMcpPost(req: IncomingMessage, res: ServerResponse): Promise<void> {
    // Stateless per-request lifecycle (decision 2): a fresh McpServer + transport
  ...
  const handler = (req: IncomingMessage, res: ServerResponse): void => {
    void (async () => {
      ...
      if (pathname === "/mcp") {
        ...
        // stateless mode: no GET SSE stream, no DELETE session teardown (decision 2)
