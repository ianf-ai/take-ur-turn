 ...
 * (decision 8), afterwards changes are edge-triggered.
 *
...
import { DEFAULT_ROLES, scriptsDir } from "./workspace.js";
 ...
 *   - labelToAgent: pane label → agent identity. Carries BOTH the agent-named
 *     panes (label == agent, the convention) and the legacy labels
 *     (workspace.json label field → that role's agent) for the transition.
 *   - roleToAgent: role → default-lineup agent (cast-less tasks).
 * routes.json is deliberately not consulted: without
 * workspace.json only the standard seats are meaningful anyway.
 */
...
async function defaultLoadRouting(): Promise<RoutingMaps> {
  const labelToAgent = new Map<string, string>();
  const roleToAgent = new Map<string, string>();
  let loaded = false;
  try {
    const raw = JSON.parse(
      await readFile(path.join(scriptsDir(), "workspace.json"), "utf8"),
    ) as { roles?: Record<string, unknown> };
    if (raw.roles !== undefined && typeof raw.roles === "object" && raw.roles !== null) {
      for (const [role, entry] of Object.entries(raw.roles)) {
        if (entry === null || typeof entry !== "object") continue;
        const { label, agent } = entry as { label?: unknown; agent?: unknown };
        if (typeof agent !== "string" || agent.length === 0) continue;
        roleToAgent.set(role, agent);
        labelToAgent.set(agent, agent); // agent-named pane
        if (typeof label === "string" && label.length > 0) labelToAgent.set(label, agent); // legacy label
        loaded = true;
      }
    }
  } catch {
    // unreadable/absent — fall through to DEFAULT_ROLES below
  }
  if (!loaded) {
    for (const [role, seat] of Object.entries(DEFAULT_ROLES)) {
      roleToAgent.set(role, seat.agent);
      labelToAgent.set(seat.agent, seat.agent);
      labelToAgent.set(seat.label, seat.agent);
    }
  }
...
export class Notifier {
  ...
  // Stall watchdog (in-memory; decision 7): updated_at is treated as an opaque
  ...
  private async compareAndAct(): Promise<void> {
    ...
    // Channel set rebuilt from /state's notify key EVERY poll (decision 6).
    ...
    if (prev === null) {
      // First successful fetch: baseline, no notifications (decision 8).
  ...
  private async diffTask(
    ...
    if (attentionRising) {
      ...
      // (decision 8). Never contains warnings content — /state has none; the
    ...
    if (flowMode !== "auto") {
      // manual: notify only — task, status, who moves next, pane name = task_id.
      await this.sendAll({
        title: `TUT ${after.task_id}: waiting for ${after.waiting_for}`,
        body: `${after.title} — status: ${after.status}; waiting for: ${after.waiting_for}; pane: ${after.task_id}`,
        task_id: after.task_id,
      });
      return;
    }
  ...
  private async autoLaunch(task: StateTask, role: string): Promise<void> {
    ...
    try {
      ...
      await this.sendAll({
        ...
        body: `${task.title} — status: ${task.status}; launched ${role} via launch.sh (pane: ${task.task_id})`,
  ...
  /**
   * Event→task mapping (agent-keyed): the event pane is resolved as
   *   (a) pane title IS a task_id (system-design 4.4 work-pane convention;
   *       validated against the snapshot, not string-guessed) → full mapping,
   *   else
  ...
  /**
   * done → immediate compare + cross-validation (decision 5): first the event
