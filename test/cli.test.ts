...
import { scriptsDir } from "../src/workspace.js";
import http from "node:http";

const SCRIPTS_DIR_PLACEHOLDER = scriptsDir();
...
describe("mode / start-next handlers (real hub)", () => {
  ...
  it("start-next extracts the role from waiting_for and runs launch.sh (TUT_DRY_RUN passthrough)", async () => {
    ...
    expect(out).toContain("(agent 'pi')"); // routes.json: executor → exec label
  ...
  it("start-next does not spawn when the launch marker append loses the version race", async () => {
    ...
    // Deterministic routing for the launch pre-check: hide workspace.json/routes.json
    // so executor resolves through DEFAULT_ROLES (pi, on PATH) regardless of
    // what a concurrently running test file does to those files.
    const hidden: Array<[string, string]> = [];
    for (const f of [path.join(SCRIPTS_DIR_PLACEHOLDER, "workspace.json"), path.join(SCRIPTS_DIR_PLACEHOLDER, "routes.json")]) {
      if (existsSync(f)) {
        renameSync(f, `${f}.racehide`);
        hidden.push([`${f}.racehide`, f]);
      }
    }
    let code: number;
    try {
