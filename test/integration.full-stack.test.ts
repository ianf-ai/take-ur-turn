...
describe("integration: full stack (serve + notifier + CLI drivers)", () => {
  it(
    "lifecycle: notification sequence follows the state machine; pane = task_id; approve and close are silent",
    async () => {
      ...
      // Every notification carries task/status/waiting_for and pane = task_id.
  ...
  it(
    ...
    async () => {
      ...
      expect(launched.body).toContain(`pane: ${id}`);
      // The notifier's launch log carries launch.sh's dry-run stdout: routes.json
      // maps executor → exec, and the prompt names the task. Dry-run may open
      // with provisioning preview/skip lines — assert on the send-text line,
  ...
  it("create --cast routes start-next to the cast agent; no-cast task stays on the default lineup", async () => {
    ...
    try {
      ...
      try {
        ...
        expect(io.out()).toContain("(agent 'codex')"); // cast routed the round to codex

        // Regression (zero migration): a no-cast task routes through the
        // default file chain (executor → pi in the committed lineup).
        await main([
        ...
        expect(io.out()).toContain("(agent 'pi')");
      } finally {
