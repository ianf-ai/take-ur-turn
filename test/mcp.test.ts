...
describe("context.publish + context.read", () => {
  ...
  it("omits the agent field on the record when not provided (decision 2b)", async () => {
    const taskId = await createTask();
    await call("context.publish", {
      task_id: taskId,
      role: "executor",
...
describe("context.decide", () => {
  ...
  it("approve after review pass moves status to approved and lands the record per decision 12", async () => {
