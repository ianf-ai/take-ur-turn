...
建任务时选定流程（create 的 `--flow` / MCP `flow` 字段，缺省 `full`），落库后不可变——选错只能 close 重建（记录不可删，AGENTS 不变量）：

- **full**（默认）：design → 实现 → review → 人审批，完整四阶段。有设计含量、值得独立 review 的任务选它。
- **direct**：repo 已有现成设计（活文档或既往任务已定方案），跳过设计阶段——建任务即 implementing；既有设计可补一条 design 记录作参考（不转态）。review 照常；设计前提被推翻（fail_design）在 direct 属表外，交人裁决 close 或补设计。
- **solo**：小改动免审——跳过 review，code_changes 直接进 pending_approval 由人拍板。只用于影响面小、验证口径明确的活；免的是 review 门，人的 approve 仍是门。
...
```
context.create {"title": "…", "description": "…", "creator": "…", "role": "architect" [, "flow": "direct"|"solo"]}
tut create --title "…" --description "…" --creator "…" --role architect [--flow direct|solo]
```

返回 `{task_id, status, version: 0}`——status 按 flow 派生：full/solo → `"designing"`，direct → `"implementing"`。记下 task_id；version 0 是任务当前版本号，首发 design 时带上它。
...
**expected_version 的正确用法**：值 = 你看到的任务当前版本——create 刚返回 version 0 就带 0；重设计轮 read 到最新记录 version 是 N 就带 N；上一次 publish 返回 version 4，下一次就带 4。带对了能抓住并发写入：别人先写了一手，你的发布会报版本冲突（MCP 返回 isError；CLI 非零退出码、stderr 首行是 VERSION_CONFLICT）——重读日志再发。不带也能写（跳过校验），但带上是更好的习惯。
...
| 建任务 | `context.create {"title": …, "description": …, "creator": …, "role": "architect"}` | `tut create --title "…" --description "…" --creator "…" --role architect` |
...
脚本化消费原始 JSON：`tut read <id> --json`、`tut list --json`。可选 `--agent` / `--model`（MCP 同名顶层字段）自述身份，供追溯。
